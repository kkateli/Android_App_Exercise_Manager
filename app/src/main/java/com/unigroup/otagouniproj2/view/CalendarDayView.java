package com.unigroup.otagouniproj2.view;
...
import com.unigroup.otagouniproj2.R;
import com.unigroup.otagouniproj2.data.IEvent;
import com.unigroup.otagouniproj2.data.IPopup;
import com.unigroup.otagouniproj2.data.ITimeDuration;
import com.unigroup.otagouniproj2.decoration.CdvDecoration;
...
* show calendar daily view with event popup
* */
public class CalendarDayView extends FrameLayout {
    ...
    private void init(AttributeSet attrs) {
        ...
        mLayoutDayView = findViewById(R.id.dayview_container);
        mLayoutEvent = findViewById(R.id.event_container);
        mLayoutPopup = findViewById(R.id.popup_container);


        mDayHeight = getResources().getDimensionPixelSize(R.dimen.dayHeight);

        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.CalendarActivity);
            try {
                mEventMarginLeft =
                        a.getDimensionPixelSize(R.styleable.CalendarActivity_eventMarginLeft,
                                mEventMarginLeft);
                mDayHeight =
                        a.getDimensionPixelSize(R.styleable.CalendarActivity_dayHeight, mDayHeight);
                mStartHour = a.getInt(R.styleable.CalendarActivity_startHour, mStartHour);
                mEndHour = a.getInt(R.styleable.CalendarActivity_endHour, mEndHour);
            } finally {
                a.recycle();
            }
        }
    ...
    /*
    *  draw popup view for each event
    * */
